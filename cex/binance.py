...
async def fetch_klines(session, base_url, symbol, tf, start_ms, end_ms=None, retries=3):
        ...
        for tf in TIMEFRAMES:
            ...
            while start_time < end_time and not stop_event.is_set():
                ...
                while start_time < window_end and not stop_event.is_set():
                    klines = await fetch_klines(session, base_url, symbol, tf, start_time, window_end)
                    if not klines:
                        break

                    ops = [
                        UpdateOne(
                            {"symbol": symbol.upper() + ("PERP" if market == "futures" else ""), "timestamp": k[0] // 1000},
                            {"$set": {
                                "symbol": symbol.upper() + ("PERP" if market == "futures" else ""),
                                "timestamp": k[0] // 1000,
                                "open": float(k[1]),
                                "high": float(k[2]),
                                "low": float(k[3]),
                ...
                # переходим к следующему годовому окну
                start_time = window_end

    print(f"✅ Загрузка {symbol.upper()} завершена. ({market})")


async def load_all(market="spot", tf_filter=None, start_date=None, concurrency=10):
    if market == "spot":
        symbols = await db.tickers.distinct("symbol")
        symbols = [s.lower() for s in symbols if s.endswith("usdt")]
    else:  # futures
        symbols = await db.tickers_futures.distinct("symbol")
        symbols = [s.lower() for s in symbols if s.endswith("usdt")]
...
async def _main():
    ...
    market = sys.argv[2].lower() if len(sys.argv) > 2 else "spot"  # spot | futures
    tf = sys.argv[3].lower() if len(sys.argv) > 3 else None
    start_date = sys.argv[4] if len(sys.argv) > 4 else None  # YYYY-MM
